      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="danger"
            @click="cancelLike(scope.row)">取消点赞</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    fetchData() {
      this.loading = true
      contentApi.getLikes(this.currentPage, this.pageSize)
        .then(response => {
        ...
        .catch(error => {
          this.$message.error('获取点赞列表失败：' + error.message)
        })
        .finally(() => {
          this.loading = false
        })
    ...
    cancelLike(row) {
      this.$confirm('确定取消点赞该内容吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        contentApi.likeContent(row.id, 0)
          .then(() => {
